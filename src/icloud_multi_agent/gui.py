...
class BackupGUI:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Entry(auth_frame, textvariable=self.password_var, show="*", width=18).grid(row=0, column=3, **padding)
        ttk.Button(auth_frame, text="Giriş Yap", command=self.on_login).grid(row=0, column=4, rowspan=2, **padding)
        ttk.Label(auth_frame, text="2FA Kodu:").grid(row=1, column=0, sticky=tk.W, **padding)
        ttk.Entry(auth_frame, textvariable=self.code_var, width=12).grid(row=1, column=1, sticky=tk.W, **padding)
    ...
    def on_login(self) -> None:
        ...
        if not password:
            messagebox.showwarning("Eksik bilgi", "Parolayı girin")
            return
